// admin.js — Admin panel router (EJS views bilan)
...
adminRouter.get('/subscribers', async (req, res) => {
  ...
  if (q)     { params.push(`%${String(q).toLowerCase()}%`); where.push(`(LOWER(username) LIKE $${params.length} OR LOWER(full_name) LIKE $${params.length} OR CAST(chat_id AS TEXT) LIKE $${params.length})`); }

  let sql = `SELECT chat_id,username,full_name,registered_at,paid_access,paid_via,paid_at,last_order_id,last_amount,access_sent_at FROM subscribers`;
...
// Broadcast (segmentlar)
adminRouter.get('/broadcast', (_req, res) => res.render('admin/broadcast', { ok:null, fail:null, total:null }));

adminRouter.post('/broadcast', express.urlencoded({extended:true}), async (req, res) => {
  ...
  if (segment==='buyers_all') {
    ...
    sql=`SELECT chat_id FROM subscribers WHERE COALESCE(paid_access,FALSE)=FALSE AND to_char(registered_at,'YYYY-MM')=$1`; params=[month];
  } else {
  ...
  // Telegramga yuborish (Node 18+ da fetch global — import shart emas)
  ...
  for (let i=0;i<rows.length;i++){
    try{
      ...
      if (!resp.ok) throw new Error(resp.description||'tg err');
      ok++;
    }catch{ fail++; }
