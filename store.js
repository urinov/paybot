...
function toDateOrNull(ms) { return ms ? new Date(Number(ms)) : null; }
function toMsOrNull(dt)   { return dt ? new Date(dt).getTime() : null; }
...
export async function upsertUser({ tg_id, username = null, full_name = null }) {
  if (!pool) return;
  // joined_at birinchi marta set, keyin o‘zgarmaydi
  await pool.query(`
...
// --- Orders ---
export async function logOrderCreated({ order_id, tg_id, provider, amount, create_time }) {
  if (!pool) return;
  // User bo‘lmasa ham insert qilib qo‘yamiz (tg_id bo‘yicha)
  await pool.query(`
...
export async function markOrderPerformed({ order_id, perform_time, transaction_id }) {
  if (!pool) return;
  // 1) orderni update
...
// Modul import qilinganda schema yaratilishini ta'minlaymiz (await shart emas)
